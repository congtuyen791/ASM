...
export class AdminProductFormComponent implements OnInit {
  ...
  onSubmit(data: any) {
    const submitData = {
      ...data,
      image_url: this.avatarBase64
    }
    if (this.id) {
      return this.productService.updateProduct(this.id, submitData).subscribe((obj) => {
        this.router.navigate(['admin/phones', this.id]);
        alert('Sửa thành công');
      })
    }
    return this.productService.createProduct(submitData).subscribe((obj) => {
